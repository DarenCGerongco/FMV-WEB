  ...
  const renderPageNumbers = () => {
    ...
    for (let i = 1; i <= totalPages; i++) {
      pages.push(
        <button 
          key={i} 
          className={`px-3 py-1 mx-1 rounded ${i === currentPage ? 'bg-blue-500 font-bold text-white' : 'bg-white border'}`}
    ...
    <div className={`${isOpen ? 'fixed inset-0 z-40 flex items-center justify-center' : 'hidden'}`}>
      ...
      <div className="bg-white absolute p-6 rounded-lg shadow-lg overflow-hidden  transform transition-all min-h-[54rem] w-[70%]">
        ...
        <div className="grid grid-cols-9 bg-gray-300 px-2 rounded">
            ...
            <h1 className='col-span-2 font-bold'>Original Price</h1>
        ...
        <div className="overflow-y-auto h-auto ">
            {products.length > 0 ? (
                ...
                <div 
                    ...
                    className="grid grid-cols-9 border-b hover:bg-gray-200 duration-100 cursor-pointer rounded p-1 px-2 bg-gray-100 mt-0.5"
                ...
                >
                    ...
                    <div className='col-span-2'>{product.original_price}</div>
                    <div className='col-span-1 text-red-600'>{product.quantity}</div>
                </div>
                ))
            ) : (
